/**
 * BEING - PENGEMBANGAN DIRI • BACKEND v2.5 • ADMIN TERPADU
...
function getContacts_(){return rows_('KONTAK').map(r=>({contactId:String(r.ContactID||''),nama:String(r.Nama||''),email:String(r.Email||''),wa:String(r.WA||''),instansi:String(r.Instansi||''),sumber:String(r.Sumber||''),pertamaMasuk:dateOnly_(r.PertamaMasuk),terakhirAktif:dateOnly_(r.TerakhirAktif),totalInteraksi:Number(r.TotalInteraksi||0),programIds:String(r.ProgramIDs||''),formulirIds:String(r.FormulirIDs||'')})).reverse()}
function upsertContact_(d){
 const email=normalizeEmail_(d.email),wa=normalizeWA_(d.wa),s=sh_('KONTAK'),m=headerMap_(s);let row=0;
 ...
 if(!row){appendObject_('KONTAK',{ContactID:id_('CNT'),Nama:String(d.nama||''),Email:String(d.email||''),WA:String(d.wa||''),Instansi:String(d.instansi||''),Sumber:String(d.sumber||''),PertamaMasuk:now_(),TerakhirAktif:now_(),TotalInteraksi:1,ProgramIDs:String(d.programId||''),FormulirIDs:String(d.formulirId||'')});return true}
 ...
 s.getRange(row,m.TerakhirAktif).setValue(now_());s.getRange(row,m.TotalInteraksi).setValue(Number(s.getRange(row,m.TotalInteraksi).getValue()||0)+1);return true
}
