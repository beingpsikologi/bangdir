/**
 * BEING - PENGEMBANGAN DIRI • BACKEND v4.0 STABIL • ADMIN TERPADU
...
function setupBeingLite(){const ss=SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);Object.keys(SHEETS).forEach(name=>{let sh=ss.getSheetByName(name);if(!sh)sh=ss.insertSheet(name);ensureHeaders_(sh,SHEETS[name])});return'Setup BEING Bangdir v4.0 selesai.'}
...
function parsePayload_(v){try{return JSON.parse(String(v||'{}'))||{}}catch(_){return{}}}
...
function doGet(e){
 ...
 try{
  if(a==='ping')return outGet_({ok:true,message:'BEING Bangdir v4.0 aktif'},p);
  if(a==='publicPrograms')return outGet_({ok:true,data:getPublicPrograms_()},p);
  if(a==='portal')return outGet_(getPortal_(p.token||'',p.programId||''),p);

  requireAdmin_(p.adminKey);

  if(a==='adminPrograms')return outGet_({ok:true,data:getPrograms_()},p);
  if(a==='adminParticipants')return outGet_({ok:true,data:getParticipants_()},p);
  if(a==='adminMaterials')return outGet_({ok:true,data:getMaterials_()},p);
  if(a==='adminSessions')return outGet_({ok:true,data:getSessions_()},p);
  if(a==='adminPayments')return outGet_({ok:true,data:getPayments_()},p);
  if(a==='adminContacts')return outGet_({ok:true,data:getContacts_()},p);

  const payload=parsePayload_(p.payload);
  const map={
   createProgram:createProgram_,updateProgram:updateProgram_,setProgramStatus:setProgramStatus_,
   setProgramMedia:setProgramMedia_,deleteProgram:deleteProgram_,
   createMaterial:createMaterial_,updateMaterial:updateMaterial_,setMaterialStatus:setMaterialStatus_,deleteMaterial:deleteMaterial_,
   createSession:createSession_,updateSession:updateSession_,setSessionStatus:setSessionStatus_,toggleSessionZoom:toggleSessionZoom_,deleteSession:deleteSession_,
   deleteParticipant:deleteParticipant_,approvePayment:approvePayment_,rejectPayment:rejectPayment_,deletePayment:deletePayment_,
   sendAccessEmail:sendAccessEmailAction_,syncContacts:syncParticipantsToContacts_,deleteContact:deleteContact_
  };
  if(map[a])return outGet_(map[a](payload),p);

  return outGet_({ok:false,message:'Action tidak dikenal'},p);
 }catch(err){return outGet_({ok:false,message:String(err.message||err)},p)}
}
function doPost(e){try{let b={};if(e&&e.parameter&&Object.keys(e.parameter).length)b=e.parameter;else if(e&&e.postData&&e.postData.contents){try{b=JSON.parse(e.postData.contents)}catch(_){}}const a=b.action||'';if(a==='register')return out(register_(b));if(a==='submitPayment')return out(submitPayment_(b));requireAdmin_(b.adminKey);
 const map={createProgram:createProgram_,updateProgram:updateProgram_,setProgramStatus:setProgramStatus_,setProgramMedia:setProgramMedia_,deleteProgram:deleteProgram_,createMaterial:createMaterial_,updateMaterial:updateMaterial_,setMaterialStatus:setMaterialStatus_,deleteMaterial:deleteMaterial_,createSession:createSession_,updateSession:updateSession_,setSessionStatus:setSessionStatus_,toggleSessionZoom:toggleSessionZoom_,deleteSession:deleteSession_,deleteParticipant:deleteParticipant_,approvePayment:approvePayment_,rejectPayment:rejectPayment_,deletePayment:deletePayment_,sendAccessEmail:sendAccessEmailAction_,syncContacts:syncParticipantsToContacts_,deleteContact:deleteContact_};
...
function getPrograms_(){return rows_('PROGRAM').map(r=>({programId:String(r.ProgramID||''),nama:String(r.Nama||''),kategori:normalizeCategory_(r.Kategori),deskripsi:String(r.Deskripsi||''),status:String(r.Status||'').trim().toUpperCase(),pricingType:pricing_(r.PricingType),price:Number(r.Price||0),tanggalMulai:dateOnly_(r.TanggalMulai),tanggalAkhir:dateOnly_(r.TanggalAkhir),selesai:hasEnded_(r.TanggalAkhir||r.TanggalMulai),tanggal:dateOnly_(r.Tanggal),mediaUrl:String(r.MediaURL||''),mediaType:inferMediaType_(r.MediaURL,r.MediaType)}))}
...
function getContacts_(){
  return rows_('KONTAK').map(r=>({
    ...
    pertamaMasuk:dateOnly_(r.PertamaMasuk),
    terakhirAktif:dateOnly_(r.TerakhirAktif),
    totalInteraksi:Number(r.TotalInteraksi||0),
    programIds:String(r.ProgramIDs||''),
    formulirIds:String(r.FormulirIDs||'')
  })).reverse();
}

function updateProgram_(b){
 const id=String(b.programId||'').trim(),nama=String(b.nama||'').trim();
 if(!id||!nama)return{ok:false,message:'Program dan nama program wajib diisi.'};
 const existing=getPrograms_().find(p=>p.programId===id);
 if(!existing)return{ok:false,message:'Program tidak ditemukan.'};
 const type=pricing_(b.pricingType),price=type==='GRATIS'?0:Number(b.price||0);
 if(type==='BERBAYAR'&&price<=0)return{ok:false,message:'Program berbayar harus memiliki harga.'};
 const tm=String(b.tanggalMulai||''),ta=String(b.tanggalAkhir||'');
 if(tm&&ta&&ta<tm)return{ok:false,message:'Sampai tanggal tidak boleh lebih awal dari tanggal kegiatan.'};
 updateById_('PROGRAM','ProgramID',id,'Nama',nama);
 updateById_('PROGRAM','ProgramID',id,'Kategori',normalizeCategory_(b.kategori));
 updateById_('PROGRAM','ProgramID',id,'Deskripsi',String(b.deskripsi||'').trim());
 updateById_('PROGRAM','ProgramID',id,'PricingType',type);
 updateById_('PROGRAM','ProgramID',id,'Price',price);
 updateById_('PROGRAM','ProgramID',id,'TanggalMulai',tm);
 updateById_('PROGRAM','ProgramID',id,'TanggalAkhir',ta);
 if(Object.prototype.hasOwnProperty.call(b,'mediaUrl'))updateById_('PROGRAM','ProgramID',id,'MediaURL',String(b.mediaUrl||'').trim());
 if(Object.prototype.hasOwnProperty.call(b,'mediaType'))updateById_('PROGRAM','ProgramID',id,'MediaType',inferMediaType_(b.mediaUrl,b.mediaType));
 return{ok:true,message:'Program berhasil diperbarui.'}}
...
function register_(b){
 ...
 upsertContact_({nama,email,wa,instansi:String(b.institution||''),sumber:'PENGEMBANGAN DIRI',programId});
...
function updateMaterial_(b){
 const id=String(b.materialId||'').trim(),pid=String(b.programId||'').trim(),sid=String(b.sesiId||'').trim();
 if(!id||!pid||!String(b.judul||'').trim()||!String(b.link||'').trim())return{ok:false,message:'Data materi belum lengkap.'};
 if(sid){
   const s=getSessions_().find(x=>x.sesiId===sid);
   if(!s)return{ok:false,message:'Sesi tidak ditemukan.'};
   if(s.programId!==pid)return{ok:false,message:'Sesi yang dipilih bukan milik program tersebut.'};
 }
 updateById_('MATERI','MateriID',id,'ProgramID',pid);
 updateById_('MATERI','MateriID',id,'SesiID',sid);
 updateById_('MATERI','MateriID',id,'Judul',String(b.judul||'').trim());
 updateById_('MATERI','MateriID',id,'Deskripsi',String(b.deskripsi||'').trim());
 updateById_('MATERI','MateriID',id,'Link',String(b.link||'').trim());
...
function updateSession_(b){
 const id=String(b.sessionId||'').trim(),pid=String(b.programId||'').trim();
 if(!id||!pid||!String(b.judul||'').trim()||!String(b.tanggalSesi||'').trim())return{ok:false,message:'Data sesi belum lengkap.'};
 const old=getSessions_().find(x=>x.sesiId===id);
 if(!old)return{ok:false,message:'Sesi tidak ditemukan.'};
 updateById_('SESI','SesiID',id,'ProgramID',pid);
 updateById_('SESI','SesiID',id,'Judul',String(b.judul||'').trim());
 updateById_('SESI','SesiID',id,'Deskripsi',String(b.deskripsi||'').trim());
 updateById_('SESI','SesiID',id,'TanggalSesi',String(b.tanggalSesi||'').trim());
 updateById_('SESI','SesiID',id,'JamSesi',String(b.jamSesi||'').trim());
 updateById_('SESI','SesiID',id,'ZoomURL',String(b.zoomUrl||'').trim());
 if(old.programId!==pid){
   const ms=sh_('MATERI'),m=headerMap_(ms);
   for(let r=2;r<=ms.getLastRow();r++){
     if(String(ms.getRange(r,m.SesiID).getValue())===id)ms.getRange(r,m.ProgramID).setValue(pid);
   }
 }
 return{ok:true,message:'Sesi berhasil diperbarui.'}
}
...
function getPortal_(token,programId){
 const t=String(token||'').trim().toUpperCase();
 if(!t)return{ok:false,message:'Masukkan kode akses.'};
 const active=getParticipants_().filter(p=>p.status==='AKTIF'&&String(p.accessToken||'').toUpperCase()===t);
 if(!active.length)return{ok:false,message:'Kode akses tidak valid atau belum ada program aktif.'};
 const ids=[...new Set(active.map(p=>p.programId))],plist=getPrograms_().filter(p=>ids.includes(p.programId));
 const selected=plist.find(p=>p.programId===String(programId||''))||plist[0];
 if(!selected)return{ok:false,message:'Program aktif tidak ditemukan.'};
 const owner=active[0],sesi=getSessions_().filter(s=>s.programId===selected.programId&&s.status==='PUBLIKASI');
 const sesiIds=new Set(sesi.map(s=>s.sesiId));
 const materi=getMaterials_().filter(m=>m.programId===selected.programId&&m.status==='PUBLIKASI'&&(!m.sesiId||sesiIds.has(m.sesiId)));
 return{ok:true,data:{peserta:{nama:owner.nama,email:owner.email,accessToken:owner.accessToken},programs:plist,program:selected,sesi,materi,refreshedAt:new Date().toISOString()}};
}

function normalizeEmail_(v){return String(v||'').trim().toLowerCase()}
function normalizeWA_(v){let r=String(v||'').replace(/\D/g,'');if(r.startsWith('0'))r='62'+r.slice(1);return r}
...
function upsertContact_(d){
 const email=normalizeEmail_(d.email),wa=normalizeWA_(d.wa),s=sh_('KONTAK'),m=headerMap_(s);
 if(!email&&!wa&&!String(d.nama||'').trim())return false;
 let row=0;
 for(let r=2;r<=s.getLastRow();r++){
  const re=normalizeEmail_(s.getRange(r,m.Email).getValue()),rw=normalizeWA_(s.getRange(r,m.WA).getValue());
  if((email&&re===email)||(wa&&rw===wa)){row=r;break}
 }
 if(!row){
  appendObject_('KONTAK',{ContactID:id_('CNT'),Nama:String(d.nama||''),Email:String(d.email||''),WA:String(d.wa||''),Instansi:String(d.instansi||''),Sumber:String(d.sumber||''),PertamaMasuk:now_(),TerakhirAktif:now_(),TotalInteraksi:1,ProgramIDs:String(d.programId||''),FormulirIDs:String(d.formulirId||'')});
  return true;
 }
 ...
 if(d.formulirId)s.getRange(row,m.FormulirIDs).setValue(mergePipe_(s.getRange(row,m.FormulirIDs).getValue(),d.formulirId));
 s.getRange(row,m.TerakhirAktif).setValue(now_());
 s.getRange(row,m.TotalInteraksi).setValue(Number(s.getRange(row,m.TotalInteraksi).getValue()||0)+1);
 return true;
}

function syncParticipantsToContacts_(){const peserta=rows_('PESERTA');peserta.forEach(r=>upsertContact_({nama:r.Nama,email:r.Email,wa:r.WA,instansi:r.Instansi,sumber:'PENGEMBANGAN DIRI',programId:r.ProgramID}));return{ok:true,message:peserta.length+' data peserta disinkronkan ke Database Kontak.'}}

function deleteContact_(b){return deleteById_('KONTAK','ContactID',String(b.contactId||''))?{ok:true,message:'Kontak dihapus dari database kontak. Data peserta/formulir sumber tidak dihapus.'}:{ok:false,message:'Kontak tidak ditemukan.'}}
...
function auditBeingBackend(){
 setupBeingLite();
 return {
  version:'4.0',
  spreadsheetId:CONFIG.SPREADSHEET_ID,
  program:getPrograms_().length,
  peserta:getParticipants_().length,
  pembayaran:getPayments_().length,
  sesi:getSessions_().length,
  materi:getMaterials_().length,
  kontak:getContacts_().length
 };
}
